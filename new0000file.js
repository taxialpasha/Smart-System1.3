    ...
    function loadInstallmentData() {
        try {
            ...
            if (savedInstallments) {
                ...
                installments = fixInstallmentDates(installments);
            ...
            if (savedPaidInstallments) {
                ...
                paidInstallments = fixInstallmentDates(paidInstallments);
...
function payInstallmentMonth(installmentId, monthlyIndex, paymentData) {
    ...
    // تحديث عرض الأقساط المدفوعة إذا كان القسط مكتملاً
    if (allPaid) {
...
function moveCompletedInstallment(installment) {
    ...
    if (index !== -1) {
        // نسخة عميقة من القسط لتجنب مشاكل الإشارة
        const completedInstallment = JSON.parse(JSON.stringify(installment));
        
        // إضافة حقل تاريخ الاكتمال إذا لم يكن موجوداً
        if (!completedInstallment.completedDate) {
            completedInstallment.completedDate = new Date().toISOString().split('T')[0];
        }
        
        // إضافة القسط إلى مصفوفة الأقساط المدفوعة
        paidInstallments.push(completedInstallment);
        
        // حذف القسط من مصفوفة الأقساط النشطة
        installments.splice(index, 1);
        
        console.log(`تم نقل القسط المكتمل "${installment.title}" إلى مصفوفة الأقساط المدفوعة`);
        
        // حفظ البيانات بعد النقل لضمان التحديث
        saveInstallmentData();
    }
}
...
function addCustomStyles() {
    ...
    function renderPaidInstallmentsTable() {
        ...
        // تطبيق البحث
        const filteredInstallments = filterPaidInstallments(paidInstallments, document.getElementById('paid-date-filter')?.value || 'all', document.getElementById('paid-installments-search')?.value || '');
