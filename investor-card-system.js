...
function searchCards(searchText) {
   ...
   // البحث في البطاقات
    ...
    const filteredCards = cards.filter(card => {
        return (
            card.investorName.toLowerCase().includes(searchText) ||
...
/**
 * الحصول على قائمة بطاقات المستثمرين من التخزين المحلي
 * @returns {Array} مصفوفة البطاقات
 */
function getInvestorCards() {
    const cardsJson = localStorage.getItem('investor_cards');
    if (!cardsJson) {
        return [];
    }
...
/**
 * حفظ قائمة بطاقات المستثمرين في التخزين المحلي
...
function saveInvestorCards(cards) {
    try {
        ...
        showNotification('حدث خطأ في حفظ بيانات البطاقات', 'error');
    }
}
...
function generateCardNumber(investorId) {
    // الأرقام التي تبدأ بها بطاقات الاستثمار (يمكن تخصيصها)
    const prefix = '6742';
    
    // استخدام جزء من معرف المستثمر
    const idPart = investorId.replace(/\D/g, '').slice(0, 8).padEnd(8, '0');
    
    // إضافة أرقام عشوائية للوصول إلى 15 رقمًا
    const randomPart = Math.floor(Math.random() * 100000).toString().padStart(5, '0');
    
    // دمج الأجزاء للحصول على 15 رقمًا
    const digits = prefix + idPart + randomPart;
...
/**
 * حساب رقم التحقق (Luhn algorithm)
 * @param {string} digits الأرقام
 * @returns {string} رقم التحقق
 */
...
/**
 * إنشاء رمز CVV عشوائي
 * @returns {string} رمز CVV
 */
...
function generateBarcode(investorId) {
    // استخدام معرف المستثمر كجزء من الرمز
    const idPart = investorId.replace(/\D/g, '').slice(0, 6).padEnd(6, '0');
    
    // إضافة الطابع الزمني
    const timestamp = Date.now().toString().slice(-10);
    
    // دمج الأجزاء
    return `INV${idPart}${timestamp}`;
}
 ...
 * @returns {string} حالة البطاقة بالعربية
 */
...
function formatCurrency(amount) {
    ...
    // استخدام دالة التنسيق من النظام الأساسي إذا كانت متاحة
...
function showNotification(message, type = 'info') {
    // استخدام دالة الإشعارات من النظام الأساسي إذا كانت متاحة
...
// تنفيذ الشيفرة عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', function() {
    ...
    InvestorCardSystem.initialize()
        .then(() => {
            ...
            // عرض البطاقات إذا كانت صفحة البطاقات نشطة
            const cardsPage = document.getElementById('investor-cards-page');
            if (cardsPage && cardsPage.classList.contains('active')) {
                InvestorCardSystem.renderInvestorCards();
                InvestorCardSystem.updateCardStatistics();
            }
        })
        .catch(error => {
...
// إضافة أسلوب CSS إضافي لدعم المزيد من ميزات البطاقة
const additionalStyles = document.createElement('style');
additionalStyles.id = 'investor-card-additional-styles';
additionalStyles.textContent = `
    /* أنماط التراكب للبطاقات غير النشطة */
    ...
    /* أنماط إضافية للبطاقة */
    .investor-card {
        user-select: none;
    }
    
    .investor-card.flipped .investor-card-back {
        box-shadow: 0 8px 20px rgba(0, 0, 0, 0.15);
    }
